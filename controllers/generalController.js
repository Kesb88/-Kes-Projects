        ...
        if (userExist) {
          ...
          newUser
            ...
            .then((userSaved) => {
              ...
              console.log(
                `User ${userSaved.firstName} has been successfully added to database..`
              );
              client.messages
                .create(DOMAIN, msg)
                .then(() => {
...
router.get("/cart", (req, res) => {
  ...
  if (removeRental) {
    const removedRental = req.session.cart.find(
      (rental) => rental.rentalId === removeRental
    );
  ...
  if (rentalId) {
    ...
    const existingRental = req.session.cart.find(
  ...
  if (req.session.cart?.length > 0) {
    rentalModel
      .find({ _id: { $in: req.session.cart.map((item) => item.rentalId) } })
      .then((cartRentals) => {
        subtotal = req.session.cart.reduce((acc, rental) => {
          const cartRental = cartRentals.find(
            (item) => item._id.toString() === rental.rentalId
          );
          if (cartRental) {
...
router.post("/cart", (req, res) => {
  ...
  rentalModel.find({ _id: { $in: rentalInfo } }).then((cartRentals) => {
    const msg = {
      to: user,
      from: "Libby's Rental Properties<bascillok@gmail.com>",
      subject: "Libby's Rental Team",
      html: `
        ...
        }, for placing your order!</h3>
                    <p>Order Receipt</p>
                    <ul>
                        ${req.session.cart.map((cartItem) => {
                          const rental = cartRentals.find(
                            (item) => item._id.toString() === cartItem.rentalId
                          );
                          return `
                                <li>
                                    ...
                                    <strong>Price per Night:</strong> $${rental.pricePerNight.toFixed(
                                      2
                                    )}<br>
                                    <strong>Total Price:</strong> $${(
                                      rental.pricePerNight * cartItem.nights
                                    ).toFixed(2)}<br>
                                </li>
                                <br>
                            `;
